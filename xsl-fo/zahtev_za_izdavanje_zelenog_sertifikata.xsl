...
<xsl:template match="/x:zahtev-za-izdavanje-zelenog-sertifikata">
  <fo:root font-family="Times New Roman">
    ...
    <fo:page-sequence master-reference="zahtev-za-izdavanje-zelenog-sertifikata-page">
      <fo:flow flow-name="xsl-region-body">
        ...
        <fo:block xsl:use-attribute-sets="h3">за издавање дигиталног зеленог сертификата</fo:block>
  ...
  подносим захтев за издавање дигиталног зеленог сертификата.</fo:block>
        ...
        <fo:block xsl:use-attribute-sets="div margin-top">
          <fo:inline xsl:use-attribute-sets="dots-underline"><xsl:value-of select="x:razlog" /></fo:inline>
        </fo:block>
        ...
        <fo:table xsl:use-attribute-sets="margin-top">
          ...
          <fo:table-body>
            <fo:table-row>
              <fo:table-cell>
                <fo:block>
                  У <fo:inline xsl:use-attribute-sets="underline"><xsl:value-of select="@mesto" /></fo:inline>,
                </fo:block>
              </fo:table-cell>
              <fo:table-cell>
                <fo:block>
            ...
            <fo:table-row>
              <fo:table-cell>
                <fo:block>
                  <fo:inline xsl:use-attribute-sets="p">дана <fo:inline xsl:use-attribute-sets="underline"><xsl:value-of select="@dan" /></fo:inline> године</fo:inline>
                </fo:block>
              </fo:table-cell>
              <fo:table-cell>
                <fo:block>
