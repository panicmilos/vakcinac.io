  ...
  <fo:root font-family="Times New Roman">
    ...
    <fo:page-sequence master-reference="izjava-interesovanja-za-vakcinisanje-page">
      <fo:flow flow-name="xsl-region-body">
        <fo:block xsl:use-attribute-sets="h3 div">Исказивање интересовања за вакцинисање против COVID-19</fo:block>
        ...
        <fo:table xsl:use-attribute-sets="margin-top">
          ...
          <fo:table-body>
            <fo:table-row>
              <fo:table-cell>
                <fo:block>
                  <fo:inline>дана <fo:inline xsl:use-attribute-sets="underline"><xsl:value-of select="@dan" /></fo:inline> године</fo:inline>
                </fo:block>
              </fo:table-cell>
              <fo:table-cell>
                <fo:block>
